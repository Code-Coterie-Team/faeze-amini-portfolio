...
function AddNewTab({ tabTitle, href }: IListTabs) {
	...
	return (
		<Link
			href={href}
			className={`title flex items-center gap-2 border-r border-b border-r-borderDarck p-2 ${href === activeTab
		...
		>
			<div>
				{iconProject.map((item) =>
					href === item.href ? <span key={item.href}>{item.iconT}</span> : null
				)}
